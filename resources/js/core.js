...
var photoBooth = (function () {
    ...
    public.errorPic = function (result) {
        setTimeout(function () {
            $('.spinner').hide();
            $('.loading').html(L10N.error + '<a class="btn" href="index.php">' + L10N.reload + '</a>');
        }, 1100);
    }
    ...
    public.renderPic = function (result) {
        ...
        $('<img src="images/' + result.img + '" class="original">').load(function () {
            ...
            if (showResultPic == false)
            {
                $('#result').css({
                    'background-image': 'url(resources/img/done.png)'
                });
            }
            else
            {
    ...
    // Take Picture Button
    $('.takePic, .newpic, .takePicBody').click(function (e) {
        e.preventDefault();
        var target = $(e.target);
